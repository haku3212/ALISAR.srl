...
const COMMANDS = {
  ...
  async fill(args) {
    ...
    const val = rest.join(' ');
    await page.evaluate(([s,v]) => { const el = document.querySelector(s); if(el){el.value=v; el.dispatchEvent(new Event('change',{bubbles:true})); el.dispatchEvent(new Event('input',{bubbles:true}));} }, [sel, val]);
    console.log('fill', sel, '=', val);
  },
  async wait(sel) {
    if (!page) return console.log('ERROR: launch first');
    try { await page.waitForSelector(sel, { timeout: 8000 }); console.log('found:', sel); }
  ...
  async text(sel) {
    if (!page) return console.log('ERROR: launch first');
    console.log(await page.evaluate(s => (s ? document.querySelector(s) : document.body)?.innerText?.slice(0,300) ?? '(null)', sel||null));
  },
  async windows() {
...
console.log('ALISAR driver ready — "help" for commands');
rl.prompt();
